  ...
  try {
    ...
    if (hasAccount === '0x0000000000000000000000000000000000000000') {
      ...
      if (eventLogs && eventLogs.length > 0) {
        const tbaAddress = eventLogs[0].args[1]; // This is your TBA address from the event
        console.log('TBA Address:', tbaAddress);
      } else {
        console.error(
          'Failed to find TBACreated event in transaction receipt.'
        );
      }
      ...
      const journalResponse2 = await journalContract.balanceOf(hasAccount);
      console.log('the journal response 2 is: ', Number(journalResponse2));

      if (Number(journalResponse2) === 0) {
