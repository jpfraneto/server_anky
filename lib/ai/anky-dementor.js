...
async function getSubsequentAnkyDementorNotebookPage(text, oldPrompts) {
  ...
  Respond with a a long string that inside has 8 unique new prompts designed to take the user on a process of self inquiry. Each prompt is separated by these two characters: %%. Remember, only one long string. Just write each prompt one after the next one, separated by %%. They are 8.
    `;
