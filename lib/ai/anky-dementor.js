...
async function getSubsequentAnkyDementorNotebookPage(text, oldPrompts) {
  ...
  Respond with a a long string that inside has unique a maximum of 8 new prompts designed to take the user on a process of self inquiry. Each prompt is separated by these two characters: %%. Remember, only one long string. Maximum 8 prompts. Just write each prompt one after the next one, separated by %%. You decide how many prompts you respond with, but they need to be separated by %%. Not ennumerated. We don't need an explanation. Just the prompts. That is very important. Use the %% only to separate between prompts. Not at the beginning. Not at the end.
    `;
