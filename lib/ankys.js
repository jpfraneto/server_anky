...
const checkAllAnkys = async () => {
  try {
    const haveNotMinted = await prisma.midjourneyOnAFrame.findMany({
      where: { userFid: 208481, alreadyMinted: false, balanceOf: "0" },
    });
    console.log("the ones that havent minted are: ", haveNotMinted);
