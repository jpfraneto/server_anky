...
const checkAndUpdateGeneratedAnkys = async () => {
  const allAnkys = await prisma.generatedAnky.findMany({});
  console.log("all the ankys are: ", allAnkys);
  const ankys = await prisma.generatedAnky.findMany({
    where: {
      OR: [
        { imagineApiStatus: "pending" },
  ...
  for (const anky of allAnkys) {
    try {
      ...
      if (apiData.status === "completed") {
        ...
        console.log("the cast was shipped", thisCastCreationResponse.data.cast);
      } else if (apiData.status === "failed") {
