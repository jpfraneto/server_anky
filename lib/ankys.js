...
const ankyOnAFrameContract = new ethers.Contract(
  "0x6fC3b06fFc12028466f8010B457A2315d6A365B5",
  ANKY_ON_A_FRAME_ABI,
  wallet
);
...
const updateWinningImageForThisAnky = async (cid) => {
  try {
    ...
    console.log("the winning image url is :", winningImageUrl);
    ...
    if (!anky.imageIPFSHash) {
      ...
      console.log("this image cid is: ", thisAnkyImageCid);
    }
...
const updateWinningImageForAnkys = async () => {
  console.log("inside the update winning image for ankys");
  const ankys = await prisma.generatedAnky.findMany({
    where: {
      // Assuming there's a field to indicate whether the winning image has been determined
      winningImageUrl: null,
      // You might need additional conditions here depending on your schema
    },
  });

  for (const anky of ankys) {
    console.log("adding the information of this anky to ipfs");
    try {
      const votes = await prisma.vote.findMany({
        where: {
          ankyCid: anky.cid,
      ...
      if (!anky.imageIPFSHash) {
        ...
        const imageBuffer = await axios
          .get(winningImageUrl, { responseType: "arraybuffer" })
          .then((res) => res.data);
        const thisAnkyImageCid = await uploadImageToPinata(
          imageBuffer,
          anky.cid
        );
      }
      if (!anky.metadataIPFSHash) {
        // Create and upload metadata to Pinata
        const thisAnkyMetadata = {
          image: `ipfs://${anky.imageIPFSHash}`,
          writingCid: anky.cid,
          description: anky.ankyBio,
          name: anky.title,
        };
        const thisAnkyMetadataCid = await uploadMetadataToPinata(
          thisAnkyMetadata
        );
        ...
        const resonseFromContract =
          await ankyOneContract.createThisAnkyBeforeOpeningMint(
            anky.cid,
            thisAnkyMetadataCid,
            222
          );
        console.log("response from contgract", responseFromContract);
        console.log("the anky is ready to be minted");
      }
    } catch (error) {
      console.error(
